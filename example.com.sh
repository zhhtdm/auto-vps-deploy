...
PROJECTS=(
    'https://github.com/xxx/server1.git|/opt/server1|server1|1234|server1|/opt/server1/.venv/bin/python3 /opt/server1/app.py|APP_PATH=/,PORT=1234,TOKEN=xxxx'
...
EOF
    ...
    ENV_FILE="$DIR/.env"
    echo "$REST" | awk -F, '{for(i=1;i<=NF;i++){gsub(/^"|"$/, "", $i); if($i!="") print $i}}' > "$ENV_FILE"
    chmod 600 "$ENV_FILE"
